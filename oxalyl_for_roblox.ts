// wlft / 2025-04-28 / intended for Cloudflare Workers / mostly typed but not fully complete/ready (still functional)

interface sys_config {
    v: string;
    build: string;
    user_agent: string;
    operating_from: string;
}

interface errors_map {
    [key: string]: string;
}

const sys: sys_config = {
    v: "0.0.3",
    build: "stable",
...
export default {
    async fetch(req, env, ctx): Promise<Response> {
        ...
        let subdomain: string|null = segments[1]
        if (subdomain === "root") { subdomain = null; };
  
        /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  
        let destination: string = `https://${subdomain || ""}${subdomain ? "." : ""}${segments[0]}/${segments.slice(2).join('/')}`
        let target_url: URL = new URL(destination)
  
        target_url.search = url.search
        target_url.hash = url.hash
  
        let pheaders: Headers = new Headers()
...
}; // potentially replace with: } satisfies ExportedHandler<Env>;
